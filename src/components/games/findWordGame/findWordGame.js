    ...
    testWord() {
        ...
        if (this.state.words.includes(word)) {
            ...
            for(let i = 0; i < rightAnswers.length; i++){
                for(let q = 0; q < rightAnswers[i].length; q++){
                    if(rightAnswers[i][q] === selectedCells[q]){
                        if(q+1 === rightAnswers[i].length){
                            isRightAnswer = true;
                        }
                    }else{
                        break;
...
function getFillWordAndPlacesWords(wordsLength, height) {
    try{
        ...
        for (let i = 0; i < wordArray.length; i++) {
            let fillWordAndArr = getWordsInArrAndPlaces(wordArray[i], fillWord);;
            fillWord = fillWordAndArr[0];
            rightAnswers.push(fillWordAndArr[1]);
        }
...
function getWordsInArrAndPlaces(word, arr) {
    ...
    for (let i = 1; i < word.length; i++) {
        ...
        for (let q = 0; q < 4; q++) {
            ...
            if(isLetterInCell){
                copyArr[thatRow][thatCol] = word[i];
                allCells.push(thatRow + '-' + thatCol);
                break;
            }
        }
        if (!isLetterInCell) {
